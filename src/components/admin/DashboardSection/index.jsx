  ...
  const generateQRCodes = async (checkoutId, participantIndex) => {
    ...
    try {
      ...
      if (response.data.success) {
        const qrRawData = response.data.data;
        if (
          !qrRawData ||
          !qrRawData["2025-05-31"] ||
          !qrRawData["2025-06-01"]
        ) {
          throw new Error("Dados de QR Code incompletos na resposta");
        }
        setQrCodeData((prev) => ({
          ...prev,
          [`${checkoutId}-${participantIndex}`]: qrRawData,
        }));
      } else {
        throw new Error(response.data.message || "Falha ao gerar QR Codes");
      }
    } catch (error) {
      const errorMsg =
        error.response?.data?.message || error.message || "Erro desconhecido";
      console.error("Erro ao gerar QR Codes:", errorMsg);
      setQrError(errorMsg);
    }
  };
                      ...
                      <li key={index}>
                        ...
                        {!qrCodeData[`${checkout.id}-${index}`] ? (
                          <>
                            <Button
                              onClick={() =>
                                generateQRCodes(checkout.id, index)
                              }
                              disabled={loading}
                              sx={{
                                color: "#1976D2",
